...
/**
 * Contains activity related methods. Examples are: getCurrentActivity(), getActivityList(), getAllOpenedActivities().
 * 
...
class ActivityUtils {
     ...
     * 
     * @param orientation
     *            An orientation constant such as {@link android.content.pm.ActivityInfo#SCREEN_ORIENTATION_LANDSCAPE} or
     ...
     * 
     * @param name
     *            the name of the {@code Activity} to return to, e.g. {@code "MyActivity"}
     * 
     */
    ...
    public void finishInactiveActivities() {
        for (Iterator<Activity> iter = activityList.iterator(); iter.hasNext();) {
            final Activity activity = iter.next();
